...
| `isSkipped` | 읽기 대상이지만 충분히 응시하지 않은 영역 여부 |
...
- `data.samples`는 디버깅과 재현에 필요한 최소 정보만 포함한다.
...
| [gaze-analysis-results.json](samples/gaze-analysis-results.json) | 공통 세션 분석 결과 저장 요청 예시 |
| [story-gaze-analysis.json](samples/story-gaze-analysis.json) | 공통 분석 API에 포함할 이야기 문장·단어별 분석 payload 예시 |
