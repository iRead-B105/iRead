...
- Backend, Frontend, AI server, 아동 앱, 시선 추적 프로토타입은 각각 `iRead-backend`, `iRead-frontend`, `iRead-ai`, `iRead-app`, `iRead-eyetracking` 공개 저장소로 관리한다.
- 서비스 저장소는 각각 `services/backend`, `services/frontend`, `services/ai`, `services/app`, `services/eyetracking`에 Git submodule로 연결한다.
- 오케스트레이션 저장소는 GitHub `iRead-B105/iRead`와 연결되어 있으며 `main`, `develop` 브랜치를 사용한다.
- 여섯 저장소는 모두 공개 저장소이며 `main`, `develop` 브랜치를 사용하고 기본 브랜치는 `develop`이다.
...
- 아동 앱: [TBD]
- 시선 추적 프로토타입: FastAPI, HTML/CSS/JavaScript, C++, Tobii Game Integration SDK
...
- 소스 코드나 기능을 수정할 때 변경 동작, 경계 조건과 주요 실패 경로를 검증하는 테스트 코드를 같은 작업에서 추가하거나 수정한다.
- 변경 범위에 직접 관련된 테스트를 반드시 실행하고 성공을 확인한다. 관련 테스트 범위를 특정하기 어렵거나 공통 기반에 영향을 주면 해당 서비스의 전체 테스트를 실행한다.
- 실패한 테스트는 원인을 해결하고 다시 실행한다. 테스트가 통과하지 않은 상태에서는 작업을 완료하거나 백로그·TODO를 `done`으로 변경하지 않는다.
- 테스트 작성이나 실행이 기술적으로 불가능하면 임의로 생략하지 않고 차단 사유와 대체 검증안을 사용자에게 설명한 뒤 확인받는다.
- 빌드, 린트, 타입 검사와 정적 분석은 프로젝트의 기존 완료 기준이나 현재 변경의 위험에 필요하면 함께 실행한다. 실행하지 않은 검증은 완료 보고에 명시한다.
- 문서·하네스 변경에는 `python tools/validate_harness.py`를 실행한다.
- 테스트 결과는 실행 명령, 성공 여부와 핵심 수치를 완료 보고와 관련 백로그 또는 PR에 기록한다.
...
- 소스 변경은 관련 테스트 코드와 테스트 실행이 성공해야 하며, 결과를 관련 백로그와 완료 보고에 기록한다.
- 구현 백로그·TODO는 작업 구현과 필수 테스트가 모두 완료된 뒤에만 `done`으로 변경한다.
