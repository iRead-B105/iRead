...
description: "ERDCloud에서 확정한 25개 테이블 설계를 MySQL 계약과 Flyway 단일 V1의 기준선으로 채택한 결정입니다."
...
- [확정 ERD 이미지](../../contracts/database/erd.png)의 25개 테이블과 관계를 현재 데이터 모델로 채택한다.
...
- 2026-07-28: 최종 ERD에 `reading_features`, `student_feature_profiles`를 포함하고 `training_templates.form`을 `prompt`로 교체했다.

## 검증

- 계약 스키마와 Flyway V1의 동일성을 검사한다.
- 생성 ERD가 25개 테이블과 34개 외래 키를 포함하는지 검사한다.
