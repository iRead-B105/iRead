...
| RP-DSP-02 | 보고서 교수자 의견 입력 | 보고서에서만 사용하는 보호자 전달용 교수자 의견을 최대 2,000자로 직접 입력한다. | server | `patch_admin_report_by_reportId_teacher_memo` |
...
| RP-SEL-03 | 보고서 기간 입력값 검증 | 보고서 생성 기간의 필수값·실재 날짜·기간 순서·미래 날짜 여부를 검증하고, 저장 보고서 필터에도 같은 날짜·순서 검증을 적용한다. | client | - |
