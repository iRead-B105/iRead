...
| SB-DSP-02 | 교수자 프로필 정보 표시 | 교수자의 프로필 이미지, 이름, 소속 기관, 읽기 전용 이메일과 성별을 표시한다. | server | `get_admin_teacher_info` |
| SB-SAVE-01 | 교수자 프로필 입력값 검증 | 수정한 프로필 이미지와 교수자 정보의 필수 입력 여부 및 허용 형식을 검증한다. | server | `patch_admin_teacher_profile`, `patch_admin_teacher_profile_image` |
| SB-SAVE-02 | 교수자 프로필 저장 | 검증된 교수자 프로필 수정사항을 저장한다. | server | `patch_admin_teacher_profile`, `patch_admin_teacher_profile_image` |
