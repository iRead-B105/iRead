...
okf_version: "0.1"
title: iRead 지식 번들
description: iRead의 제품, 아키텍처, 결정, 계획과 워크플로 문서를 탐색하는 OKF 루트 인덱스입니다.
---
# iRead 지식 번들

이 디렉터리는 Open Knowledge Format(OKF) v0.1 지식 번들이다. 개념 문서는 YAML frontmatter와 Markdown 본문으로 구성하며, `index.md`와 `log.md`는 예약 파일로 사용한다.
...
- 개념 문서는 OKF frontmatter를 유지하고, 디렉터리 구조와 교차 링크로 관계를 표현한다.
