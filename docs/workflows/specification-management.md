...
- `tools/validate_harness.py`: 핵심 진입 파일, 내부 링크, 기록 문서 메타데이터와 ADR 등록
...
동작 변경이나 검증 공백이 있으면 관련 테스트 코드를 추가·수정하고 테스트를 실행한다. 기존 테스트가 충분하면 관련 테스트 실행만으로 완료할 수 있다. 하네스 검증은 문서 구조 검사이므로 문서 변경 시 함께 실행한다.
